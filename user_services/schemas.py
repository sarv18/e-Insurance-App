...
class UserRegistrationSchema(BaseModel):
    ...
    def validate_name(cls, value):
        '''
        Description: This method validates the full_name field.
        Parameters: 
        cls: The class the method is attached to.
        value: The value of the field (full name) that will be validated.
...
# Schema for Customer with additional dob field
class CustomerRegistrationSchema(UserRegistrationSchema):
    '''
    Extends the UserRegistrarionSchema to include dob for customers only.
    '''
    agent_id: int
    date_of_birth: date
    ...
    def validate_dob(cls, value):
        '''
        Validates the date of birth to ensure the user is at least 18 years old.
        '''
        try:
            today = date.today()
...
class PurchasePolicySchema(BaseModel):
    policy_id: int
    
class CalculatePremiumSchema(BaseModel):
    rate_of_interest: int
    
class CalculateCommissionSchema(BaseModel):
    agent_id: int
    commission_rate: int
